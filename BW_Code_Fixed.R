...
resultsAOV=aov(Treatment~Pre,data=datum)
summary(resultsAOV)
TukeyHSD(resultsAOV)

library(ggplot2)
library(ggpubr)
ggplot(data = datum, aes(Treatment,Pre)) +
  geom_boxplot(size = 1) +
  geom_boxplot(color="black", fill="gray", alpha=0.4) +
  labs(title = "MV2-25 Body Weight across Caloric Densities",
       y = "Body Weight (mg)", x = "Caloric Density") +
  theme_bw()+
 stat_compare_means(method="anova",label.y=.65,color="red")
