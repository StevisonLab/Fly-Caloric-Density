...
resultsAOV=aov(Treatment~Age,data=datum)
summary(resultsAOV)
TukeyHSD(resultsAOV)

library(ggplot2)
library(ggpubr)
ggplot(data = datum, aes(Treatment,Age)) +
  geom_boxplot(size = 1) +
  geom_boxplot(color="black", fill="gray", alpha=0.4) +
  labs(title = "MV2-25 Lifespan across Caloric Densities",
       y = "Age (d)", x = "Caloric Density") +
  theme_bw()+
 stat_compare_means(method="anova",label.y=115,label.x=0.75, color="red")