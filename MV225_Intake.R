...
resultsAOV=aov(Treatment~Spec,data=datum)
summary(resultsAOV)
TukeyHSD(resultsAOV)

library(ggplot2)
library(ggpubr)
ggplot(data = datum, aes(Treatment,Spec)) +
  geom_boxplot(size = 1) +
  geom_boxplot(color="black", fill="gray", alpha=0.4) +
  labs(title = "MV2-25 Intake across Caloric Densities",
       y = "Spec Value", x = "Caloric Density") +
  theme_bw()+
 stat_compare_means(method="anova",label.y=0.12,color="red")
